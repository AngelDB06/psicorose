  ...
  const checkSessions = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setHasSessions(appointments.some((a) => a.status === 'completed'));
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(data.message || t('common.error_connection'));
      }
    } catch {
      ...
      <div className="max-w-4xl mx-auto space-y-12">
        ...
        <div className="text-center space-y-4">
          <h1 className="text-4xl md:text-5xl font-bold text-slate-900 tracking-tight">
            {t('reviews.title')} <span className="text-primary-600">{t('reviews.title_highlight')}</span>
          </h1>
        ...
        {user ? (
          hasSessions ? (
            <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-100">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-slate-700">{t('reviews.rating_label')}</label>
                    ...
                    {[1, 2, 3, 4, 5].map((num) => (
                      <button
                        ...
                        className={`w-12 h-12 rounded-xl text-xl transition-all ${
                          rating >= num
                            ? 'bg-primary-500 text-white scale-110 shadow-lg shadow-primary-200'
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-slate-700">{t('reviews.comment_label')}</label>
                  <textarea
                    value={comment}
            ...
            <div className="bg-amber-50 rounded-3xl p-8 text-center border border-amber-100">
              <p className="text-amber-800 font-semibold">⚠️ {t('reviews.no_sessions_warning')}</p>
              <p className="text-amber-600 text-sm mt-2">{t('reviews.no_sessions_cta')}</p>
            </div>
          )
        ) : (
          <div className="bg-primary-50 rounded-3xl p-8 text-center border border-primary-100">
            <p className="text-primary-800 font-semibold mb-4">{t('reviews.login_prompt')}</p>
            <div className="flex justify-center gap-4">
              <a href="/login"   className="px-6 py-2 bg-primary-600 text-white rounded-xl font-bold hover:bg-primary-700 transition-colors">{t('reviews.login_btn')}</a>
              <a href="/registro" className="px-6 py-2 bg-white text-primary-600 border border-primary-200 rounded-xl font-bold hover:bg-primary-50 transition-colors">{t('reviews.register_btn')}</a>
            </div>
          </div>
        )}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {loading ? (
            [1, 2, 3, 4].map((i) => (
              <div key={i} className="h-48 bg-white rounded-3xl animate-pulse shadow-sm border border-slate-100" />
            ))
          ) : reviews.length === 0 ? (
            <p className="col-span-2 text-center text-slate-500 py-12">{t('reviews.empty_reviews')}</p>
          ) : (
            reviews.map((review) => (
              <div key={review._id} className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 hover:shadow-md transition-shadow relative overflow-hidden group">
                ...
                <div className="flex flex-col h-full space-y-4">
                  <div className="text-amber-400 text-sm tracking-widest">{renderStars(review.rating)}</div>
                  <p className="text-slate-700 font-medium leading-relaxed flex-grow italic">"{review.comment}"</p>
