...
function BookingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/dashboard', { state: { successMessage: '¡Cita reservada con éxito! Te esperamos.' } });
    } catch (err) {
  ...
  // Día de la semana para mostrar en el calendario
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-primary-50/30 to-slate-50 py-12 px-6">
      ...
      <div className="max-w-2xl mx-auto relative">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-bold text-slate-800">Reservar Cita</h1>
          <p className="text-slate-500 mt-2">
            Hola, <span className="font-semibold text-primary-600">{user?.name.split(' ')[0]}</span>. Elige la fecha y hora que mejor te venga.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-3 uppercase tracking-wider">
                1. Tipo de consulta <span className="text-red-400">*</span>
              </label>
            ...
            <div>
              <label htmlFor="date" className="block text-sm font-bold text-slate-700 mb-3 uppercase tracking-wider">
                2. Fecha <span className="text-red-400">*</span>
              </label>
              ...
              {formData.date && (
                <p className="mt-2 text-sm text-primary-600 font-semibold flex items-center gap-1.5">
                  ...
                  {getWeekDay(formData.date)}, {new Date(formData.date + 'T00:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
              )}
            </div>
            ...
            <div>
              <label className="block text-sm font-bold text-slate-700 mb-3 uppercase tracking-wider">
                3. Hora <span className="text-red-400">*</span>
              </label>
            ...
            <div>
              <label htmlFor="notes" className="block text-sm font-bold text-slate-700 mb-3 uppercase tracking-wider">
                4. Notas adicionales <span className="text-slate-400 font-normal normal-case">(opcional)</span>
              </label>
              <textarea
                id="notes"
                name="notes"
            ...
            {formData.reason && formData.date && formData.time && (
              <div className="bg-primary-50 border border-primary-100 rounded-2xl p-5">
                <p className="text-sm font-bold text-primary-800 mb-3">Resumen de tu cita:</p>
                <div className="space-y-1 text-sm text-primary-700">
                  <p>📋 <span className="font-semibold">{formData.reason}</span></p>
                  <p>📅 {getWeekDay(formData.date)}, {new Date(formData.date + 'T00:00:00').toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                  <p>🕐 {formData.time}h</p>
                </div>
              </div>
            )}
