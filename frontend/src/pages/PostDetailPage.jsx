  ...
  if (error || !post) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center px-6 text-center">
        ...
        <h1 className="text-3xl font-bold text-slate-800 mb-3">{t('post.not_found')}</h1>
        <p className="text-slate-500 mb-8">{error || t('post.not_found_body')}</p>
        <Link to="/blog" className="bg-primary-500 hover:bg-primary-600 text-white px-8 py-3 rounded-full font-semibold transition-all">
          {t('post.back_to_blog')}
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-4xl mx-auto px-6">
        <div className="flex items-center gap-4 py-6 border-b border-primary-100 text-sm text-slate-500">
          ...
          <span>{new Date(post.createdAt).toLocaleDateString(locale, { day: 'numeric', month: 'long', year: 'numeric' })}</span>
          <span className="w-1 h-1 rounded-full bg-slate-300" />
          <span>{post.readTime} {t('post.read_time')}</span>
        </div>
      </div>
      ...
      <article className="max-w-4xl mx-auto px-6 py-10">
        <div className="space-y-6">
          {post.content.map((paragraph, i) => (
            <p key={i} className={`text-slate-600 text-lg leading-relaxed ${i === 0 ? 'text-xl text-slate-700 font-medium' : ''}`}>
              {paragraph}
            </p>
          ))}
        </div>
        ...
        <div className="mt-14 p-8 rounded-3xl bg-gradient-to-r from-primary-50 to-blue-50 border border-primary-100 text-center">
          <p className="text-primary-900 font-bold text-xl mb-2">{t('post.cta_title')}</p>
          <p className="text-slate-500 mb-6">{t('post.cta_body')}</p>
          <Link
            to="/reservar"
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        <Link to="/blog" className="inline-flex items-center gap-2 text-sm font-semibold text-primary-600 hover:text-primary-700 transition-colors">
